(function ($) {
    ...
    var slider = function (container, options) {
        ...
        var $firstSlideClone = $sliderList
            .children()
            .slice(slidesCount - setting.items)
            .clone();
        var $lastSlideClone = $sliderList
            .children()
            .slice(0, setting.items)
            .clone();
        ...
        var init = function () {
            ...
            $sliderList.css(
                "transform",
                "translate(" + -(slideWidth * setting.items) + "px)"
            );
            attachHandlers();
            if (!setting.loop) {
        ...
        var createControlNavItems = function () {
            ...
            for (var i = 0; i < slidesCount; i++) {
                $sliderControlNavItem = $(
                    '<div class="slider_control-nav-item">'
                );
                $sliderControlNav.append($sliderControlNavItem);
            }

            $sliderControlNavList = $sliderControlNav.find(
                ".slider_control-nav-item"
            );
                ...
                if (arg === "next") {
                    if (currentSlide < slidesCount - 1) {
                        ...
                        $sliderList.css(
                            "transform",
                            "translate(" +
                                -(currentSlide + setting.items) * slideWidth +
                                "px)"
                        );
                    } else if (
                        setting.loop &&
